    ...
    <div className="bg-white rounded-xl shadow-sm p-6">
      ...
      {stats && (
        <div className="grid grid-cols-4 gap-4 mb-6">
          <div className="bg-blue-50 rounded-lg p-3">
            <div className="text-2xl font-bold text-blue-600">{stats.totalStats?.totalActivities || 0}</div>
            <div className="text-xs text-blue-600">Total Activities</div>
          </div>
          <div className="bg-green-50 rounded-lg p-3">
            <div className="text-2xl font-bold text-green-600">{stats.totalStats?.uniqueStudentCount || 0}</div>
            <div className="text-xs text-green-600">Active Students</div>
          </div>
          <div className="bg-purple-50 rounded-lg p-3">
            <div className="text-2xl font-bold text-purple-600">{stats.totalStats?.avgActivitiesPerStudent || 0}</div>
            <div className="text-xs text-purple-600">Avg Activities/Student</div>
          </div>
      ...
      {activities.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="mt-1 text-sm text-gray-500">Student activity will appear here once students start engaging with the platform.</p>
        </div>
      ) : (
        <div className="space-y-4">
          {activities.map((studentActivity, index) => (
            <div key={studentActivity.student.id} className="border-l-4 border-indigo-200 bg-gray-50 rounded-r-lg p-4">
              {/* Student Header */}
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h4 className="font-medium text-gray-900">{studentActivity.student.name}</h4>
                    <p className="text-sm text-gray-500">{studentActivity.student.email}</p>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-sm text-gray-500">{formatTimeAgo(studentActivity.lastActivity)}</div>
                  <div className="text-xs text-gray-400">{studentActivity.activities.length} activities</div>
                </div>
              </div>

              {/* Activities */}
              <div className="space-y-2">
                {studentActivity.activities.slice(0, 3).map((activity, actIndex) => (
                  <div key={activity.id} className="flex items-center space-x-3 text-sm">
                    <div className="flex-shrink-0">
                      {getActivityIcon(activity.type)}
                    </div>
                    <div className="flex-1">
                      <span className="text-gray-700">{getActivityDescription(activity)}</span>
                      <span className="text-gray-400 ml-2">• {formatTimeAgo(activity.timestamp)}</span>
                    </div>
                ...
                {studentActivity.activities.length > 3 && (
                  <div className="text-sm text-gray-500 pl-8">
                    +{studentActivity.activities.length - 3} more activities
                  </div>
                )}
              </div>
            </div>
